...
class AlunoHelper{
  ...
  "c_id INTEGER PRIMARY KEY, c_nome TEXT, c_email TEXT," 
  "c_telefone TEXT, c_instituicao TEXT, c_path_img TEXT)";
    ...
    Future<Aluno> selectById(int id) async{
      Database dbAluno = await db;
      List<Map> maps = await dbAluno.query(
        "aluno",
        columns: ["c_id", "c_nome", "c_email", "c_telefone", "c_instituicao", "c_path_img"],
        where: "c_id = ?",
        whereArgs: [id]
      );
      if(maps.length>0){
    ...
    Future<int> update(Aluno aluno) async{
      Database dbAluno = await db;
      return await dbAluno.update("aluno", aluno.toMap(), where: "c_id = ?", whereArgs: [aluno.id]);
    }
